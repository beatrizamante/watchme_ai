...
def split_dataset(images_dir, labels_dir, output_dir, train_ratio=0.7, val_ratio=0.15):
    for split in ["train", "val", "test"]:
        ...
        (Path(output_dir) / "annotations" / split).mkdir(parents=True, exist_ok=True)
    ...
    for split, files in splits.items():
        for img_file in files:
            ...
            shutil.copy(os.path.join(labels_dir, label_file), Path(output_dir) / "annotations" / split / label_file)